...
const AdminCart = () => {
  ...
  useEffect(() => {
    getServiceBooking().then((data) => {
      if (data) setServiceBookingList(data);
      else setServiceBookingList([]);
    });
  });
  ...
  const handleRemove = async (id) => {
    if (window.confirm("co muon xoa") == true) {
      axios
        .delete(`https://localhost:7024/api/typebookings/${id}`)
        .then((result) => {
  ...
  const handleEdit = (id) => {
    handleShow();
    axios
      .get(`https://localhost:7024/api/typebookings/${id}`)
      .then((result) => {
  ...
  const handleUser = (id) => {
    axios
      .get(`https://localhost:7024/api/users/${id}`)
      .then((result) => {
        setId(id);
      })
      .catch((error) => {
                          ...
                          <Modal.Body>
                            ...
                            <Col>
                              <input
                                ...
                                onChange={(e) =>
                                  setEditServiceTypeId(e.target.value)
                                }
                              />
                            </Col>
                            <Col>
                              <input
                                ...
                                value={editBookingId}
                                onChange={(e) =>
                                  setEditBookingId(e.target.value)
                                }
                              />
                            </Col>
                            {/* <Col>
                            <input type="checkbox" 
                            checked={editBookingId === 1 ? true : false}
                            onChange={(e)=> handleActiveChange(e)} value={editBookingId}/>
